...
export class Dashboard implements OnInit {
  ...
  recentOrders = computed(() =>
    StatsUtil.recentOrders(this.orders(), 5)
  );
  ...
  chartData = computed(() => {
    ...
    const latestDate = new Date( Math.max( ...orders.map(order => new Date(order.createdAt).getTime())));
    const days: Date[] = [];
    ...
    return days.map(date => {
      const dayOrders = orders.filter(order => {const orderDate = new Date( order.createdAt );
        return (
          orderDate.getFullYear() === date.getFullYear() && orderDate.getMonth() === date.getMonth() && orderDate.getDate() === date.getDate()
        );
      });

      const sales = dayOrders.reduce((sum, order) => sum + (Number(order.total) || 0), 0);
      return {
        date, label: date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
        sales, orders: dayOrders.length
      };
    });
  });
  ...
  chartSalesLabels = computed(() => {
    ...
    return [
      0, maxSales * 0.33, maxSales * 0.66, maxSales
    ];
  });
  ...
  /* SALES PATH */
  salesPath = computed(() => {
    const points = this.chartPoints().map(
        point => ({ x: point.x, y: point.salesY })
      );
  ...
  private loadStoreSettings(): void {
    this.storeSettingsService
      ...
      .subscribe({ next: (settings) => {
        ...
        error: (error) => { console.error( 'Error loading store settings:', error );
        }
      });
  }
  ...
  ngOnInit(): void {
    ...
    .subscribe({ next: ({ products, orders }) => {
