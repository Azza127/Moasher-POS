  ...
  loadInvoices(): void {
    this.purchaseInvoiceService
      ...
      .subscribe({ next: (invoices) => {
        ...
        error: (error: unknown) => { console.error( 'Failed to load purchase invoices:', error );
          this.popupService.showAlert( 'Failed to load purchase invoices.', 'error', 'Loading Error');
        }
      });
  }
...
loadCategories(): void {
  ...
  scanProductByCode(code: string): void {
    ...
    const product =
      this.products.find( item => item.sku.trim().toLowerCase() === normalizedCode );
...
saveQuickProduct(): void {
          ...
          (latestProducts) => {
            ...
            changedProducts.forEach(
              ([productId, quantityDelta]) => {
                ...
                updates.push(
                  this.updateProductStock( product, Number(quantityDelta)
                  )
                );
              }
            );
  ...
  get filteredInvoices(): PurchaseInvoice[] {
    const term =this.invoiceSearchTerm
        .trim()
        .toLowerCase();
