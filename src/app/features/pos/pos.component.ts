...
saveCart(): void {
  localStorage.setItem(
    this.CART_STORAGE_KEY, JSON.stringify(this.cart)
  );
}
...
loadTicketNumber(): void {
  ...
  if (savedTicket) {
    this.currentTicketNumber = savedTicket;
  } else {
...
loadPaymentMethod(): void {
  ...
  if (
    savedPayment === 'Cash' || savedPayment === 'Card'
  ) {
...
restoreCartStock(): void {
  ...
  checkout(): void {
    ...
    this.orderService .createOrder(newOrder as any)
      .subscribe({ next: () => {
          const updateRequests = this.cart
              .map((item) => { const product = this.products.find((p) => String(p.id) === String(item.productId));
                if (product) {
