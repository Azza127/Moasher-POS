...
export class Reports implements OnInit {
  ...
  /*  CATEGORY COLORS, KEEPING YOUR CURRENT CATEGORY COLORS*/
  ...
  private calculateReports(): void {
    const allCompleted = this.orders.filter(
        order =>
  ...
  private calculateCategorySales(
    ...
    completedOrders.forEach(
      order => {(order.items || []).forEach( item => {
            ...
            categorySalesMap.set(categoryId, currentTotal + (item.total || 0));
          }
        );
      }
    );
    ...
    this.categoryReports = this.categories.map(category => {
          ...
          const percentage =
            this.totalSales > 0 ? ( sales / this.totalSales) * 100 : 0;
      ...
      order => {
        (order.items || []).forEach( item => {
            ...
            productStatsMap.set( productId,
              { name: item.productName || existing.name, units: existing.units + (item.quantity || 0),
                revenue: existing.revenue + (item.total || 0) }
            );});});

    this.topProducts = Array.from( productStatsMap.entries() )
        .map(([id, stat]) => { const sharePercentage = this.totalSales > 0 ? ( stat.revenue / this.totalSales ) * 100 : 0;
  ...
  private calculateInventoryValuation(): void {
    ...
    const profitMargin = potentialRevenue > 0 ? (( potentialRevenue - totalCost ) / potentialRevenue ) * 100 : 0;
    this.inventoryValuation = {totalCost,potentialRevenue,totalItemsInStock,profitMargin: Number(profitMargin.toFixed(1))
    };
  }
  ...
  exportCSV(): void {
    ...
    this.categoryReports.forEach(
      category => { csvContent += `Category Name,Total Sales (${this.currencySymbol}),Percentage (%)\n`;}
    );

    csvContent += `\nProduct Name,Units Sold,Revenue (${this.currencySymbol})\n`;
    this.topProducts.forEach(
      product => { csvContent += `"${product.productName}",` + `${product.unitsSold},` + `${product.revenue}\n`; }
    );
          ...
          options: {  responsive: true, maintainAspectRatio: false,
            ...
            plugins: {
              ...
              tooltip: {
                ...
                displayColors: false,
                callbacks: {
                  label:(context) => { const value = context.parsed.y ?? 0;
            ...
            scales: {
              x: {
                ...
                ticks: { color: '#8A8178', font: { size: 11}, padding: 8}
                },

              y: {
                beginAtZero: true,
                grid: { color: '#E4DED4', lineWidth: 1 },
                border: { display: false },
                ticks: { color: '#8A8178', font: { size: 11}, padding: 8,
                    callback: (value) => {
  ...
  private renderDoughnutChart(): void {
    ...
    const centerTextPlugin = {
      id: 'centerText',
      beforeDraw:
        (chart: any) => { const { width, height, ctx } = chart;
          ctx.save();

          const formattedSales = this.totalSales >= 1000 ? `${( this.totalSales / 1000 )
            .toFixed(0)}k ${this.currencySymbol}` : `${this.totalSales
              .toFixed(0)} ${this.currencySymbol}`;
    ...
    this.doughnutChart = new Chart(
        canvas,
        {
          ...
          data: { labels, datasets: [{ data,
                ...
                backgroundColor: hasCategoryData ? this.categoryReports
                .map((_, index) => this.getCategoryColor(index)): ['#D5D8D0'],
                borderWidth: 3,
                borderColor: '#FCFCF8',
                hoverOffset: 4
              }
